...
def load_shipment_rev(file_bytes):
    ...
    for c in df.columns:
        ...
        elif 'ttl plan' in cl or 'ttl.plan' in cl: rename_map[c] = 'plan_ttl'
        elif 'o/stock' in cl or 'o.stock' in cl: rename_map[c] = 'stock_cur'
    df = df.rename(columns=rename_map)
    if 'plan_ttl' in df.columns: df = df.rename(columns={'plan_ttl': 'pred_plan'})
    if 'stock_cur' in df.columns: df = df.rename(columns={'stock_cur': 'cur_stock'})
    plan_date_cols = []
    for c in df.columns:
        cl = str(c).lower().strip()
        if 'plan' in cl and any(ch.isdigit() for ch in cl):
            if 'ttl' in cl or 'actual' in cl: continue
    ...
    if 'cur_stock' in df.columns: df = df.rename(columns={'cur_stock': 'stock_base'})
    if 'pred_plan' in df.columns: df = df.rename(columns={'pred_plan': 'plan_pred'})
    # 컬럼명 한글로
    col_rename2 = {}
    for c in df.columns:
        if c == 'stock_base': col_rename2[c] = 'stock_base'
        if c == 'plan_pred': col_rename2[c] = 'plan_pred'
    df = df.rename(columns=col_rename2)
    # 기존 한글 컬럼명으로 재매핑
    for c in df.columns:
        cl = str(c).lower().strip()
        if 'o/stock' in cl or 'o.stock' in cl or 'stock_base' == c:
            df = df.rename(columns={c: 'stock_base'}); break
    for c in df.columns:
        cl = str(c).lower().strip()
        if 'ttl plan' in cl or 'ttl.plan' in cl or 'plan_pred' == c:
            df = df.rename(columns={c: 'plan_pred'}); break
    # 최종 한글 컬럼 통일
    if 'stock_base' in df.columns: df = df.rename(columns={'stock_base': 'cur_stock_final'})
    if 'plan_pred' in df.columns: df = df.rename(columns={'plan_pred': 'pred_plan_final'})
    if 'cur_stock_final' in df.columns: df = df.rename(columns={'cur_stock_final': 'cur_stock'})
    if 'pred_plan_final' in df.columns: df = df.rename(columns={'pred_plan_final': 'pred_plan'})
    # 한글 이름으로 최종 통일
    final_rename = {}
    for c in df.columns:
        if c == 'cur_stock': final_rename[c] = 'cur_stock'
        if c == 'pred_plan': final_rename[c] = 'pred_plan'
    # Infor model 보정
...
def analyze(ship_db, plan_date_cols, note_dict, prod_db=None):
    ...
    # 컬럼 감지
    stock_col = next((c for c in ['cur_stock', 'stock_base', 'cur_stock_final'] if c in mdf.columns), None)
    plan_col  = next((c for c in ['pred_plan', 'plan_pred', 'pred_plan_final'] if c in mdf.columns), None)
    if stock_col: mdf = mdf.rename(columns={stock_col: 'cur_stock'})
    if plan_col:  mdf = mdf.rename(columns={plan_col:  'pred_plan'})
    if 'cur_stock' not in mdf.columns: mdf['cur_stock'] = 0
    if 'pred_plan' not in mdf.columns: mdf['pred_plan'] = 0

    for col in ['PO', 'pred_plan', 'cur_stock', '_TTLShip'] + plan_date_cols:
    ...
    mdf['cur_stock'] = mdf['cur_stock'].astype(int)
    mdf['pred_plan'] = mdf['pred_plan'].astype(int)

    # 표시용 한글 컬럼
    mdf['현재재고'] = mdf['cur_stock']
    mdf['예상계획'] = mdf['pred_plan']

    if 'code' not in mdf.columns: mdf['code'] = mdf['ERP']
    mdf['code'] = mdf['code'].astype(str).str.strip()

    code_stock   = mdf.groupby('code')['cur_stock'].max().to_dict()
    code_plan    = mdf.groupby('code')['pred_plan'].max().to_dict()
    ...
    if has_prod:
        ...
        if erp_col:
            ...
            mdf['실적차이']  = (mdf[f'현재실적({today_str})'] + mdf['_미래계획']) - mdf['예상계획']
    ...
    if has_prod:
        int_cols += ['예상제품재고_실적', 'Cutoff시점재고', '실적차이', f'현재실적({today_str})']
    for col in int_cols:
        if col in mdf.columns:
            mdf[col] = mdf[col].astype(int)

    mdf = mdf.drop(columns=['_부족1', '_부족2', '_cdt', '_미래계획', 'cur_stock', 'pred_plan'], errors='ignore')
...
def render_shipment_alert_tab():
    ...
    if not urgent.empty:
        ...
        cc += ['순서', 'Cut off Cargo', 'Cus', 'model', 'code', 'ERP', 'MODEL_TYPE', 'PO', '현재재고', '예상계획']
    ...
    s1c = ['알람', '순서', 'Cut off Cargo', 'Cus', 'model', 'code', 'ERP', 'MODEL_TYPE', 'PO', '현재재고', '예상계획', '예상제품재고', 'Note']
    ...
    if has_prod:
        ...
        st.caption("Cutoff시점재고 = 현재재고 + 기준일 이후 실적 - 누적PO")
        ...
        s2c = ['알람', '순서', 'Cut off Cargo', 'Cus', 'model', 'code', 'ERP', 'MODEL_TYPE',
