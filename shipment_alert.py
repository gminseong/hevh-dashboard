"""
[한솔테크닉스 HEVH] Shipment Cut-off 알람 v30.9
- GitHub DB 저장/로드
- 디버그 제거
- 사이드바 업로드
"""
    ...
    try:
        ...
        if r.status_code == 200:
            return pd.read_excel(
                io.BytesIO(base64.b64decode(r.json()["content"])))
        return pd.DataFrame()
    except: return pd.DataFrame()


def _gh_save_xlsx(file_bytes, filename):
    token, repo = _gh_config()
...
def analyze(ship_db, prod_db, plan_date_cols, note_dict):
    ...
    if erp_col is None:
        st.error(f"❌ ERP 컬럼 없음: {list(prod_db.columns)}")
        return pd.DataFrame(), ''
    ...
    mdf = mdf.drop(columns=[
        '_부족1','_부족2','_실적','_cdt','_미래계획'], errors='ignore')
...
def render_shipment_alert_tab():
    st.markdown("#### 🚨 Shipment Cut-off 알람 v30.9")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        if st.button("🗑️ 출하계획 초기화", use_container_width=True, key="rs_v30"):
            for k in ['ship_db','ship_updated','plan_date_cols','note_dict']:
                st.session_state.pop(k, None)
            st.cache_data.clear(); st.rerun()
        if st.button("🗑️ 생산실적 초기화", use_container_width=True, key="rp_v30"):
    ...
    if ship_db.empty or prod_db.empty:
        st.info("👈 사이드바에서 출하계획(.xlsx)과 생산실적(.csv)을 업로드하세요.")
        return
    ...
    if not urgent.empty:
        ...
        k2.metric("🔴 출하불가", int((urgent['알람_실적']=='🔴 출하불가').sum()))
        k3.metric("🟠 부족",     int((urgent['알람_실적']=='🟠 부족').sum()))
        cc = ['알람_계획','알람_실적','순서','Cut off Cargo','Cus','model',
              'code','ERP','MODEL_TYPE','PO','현재재고','예상계획',
              actual_col,'실적차이','예상제품재고_계획','예상제품재고_실적',
              'Cutoff시점재고','Note']
