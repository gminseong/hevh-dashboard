"""
[한솔테크닉스 HEVH] LOSSTIME + SCRAP 분석 대시보드 v4.0
- UI 전면 개선 (헤더/KPI/사이드바 정리)
- GitHub 자동 저장/로드
...
.main-header h3 { margin:0; font-size:22px; font-weight:700; }
...
.kpi-val { font-size:20px; font-weight:700; color:#1e3a5f; line-height:1.2; }
...
.pm-p1 { background:#fef2f2; border-left:4px solid #dc2626;
          padding:8px 12px; border-radius:4px; margin:3px 0; font-size:14px; }
...
def login_page():
    ...
    <div style="text-align:center;margin-top:80px;">
        <h2>🏭 한솔테크닉스 HEVH</h2>
        <p style="color:#64748b;font-size:15px;">LOSSTIME + SCRAP 분석 대시보드</p>
        ...
        if loss_row:
            ...
            for idx,slot in enumerate(slots):
                ...
                if lv>0:
                    ...
                    records.append({"date":date_str,"shift":shift,"process":process,
...
def parse_scrap_file(uploaded_file):
    ...
    for _,row in df.iterrows():
        try:
            ...
            records.append({"work_date":work_date,"process":process,"line":start_line,
...
def dashboard():
    ...
    with st.sidebar:
            ...
            if uploaded:
                if st.button("🚀 분석 / 누적",type="primary",use_container_width=True):
                    ...
                    if not nl.empty:
                        ...
                        st.success(f"✅ {len(nl):,}건 {'GitHub' if ok else '로컬'}")
                    if not ns.empty:
        ...
        st.caption("📅 날짜")
        if len(dates)>=2:
            date_range=st.select_slider("",options=dates,value=(def_s,def_e),
                                        label_visibility="collapsed")
        else:
            date_range=(dates[0],dates[0])

        st.caption("🏭 공정")
        procs=st.multiselect("",["AI","SMT","MI"],default=["AI","SMT","MI"],
                             label_visibility="collapsed")

        st.caption("🌙 주야간")
        shifts=st.multiselect("",["DAY","NIGHT"],default=["DAY","NIGHT"],
                              label_visibility="collapsed")
        ...
        # 추가 필터 없으면 기본값
        if "sel_lines" not in dir(): sel_lines=[]
        if "sel_types" not in dir(): sel_types=[]
        if "view_mode" not in dir(): view_mode="TOTAL(일계)"
    ...
    # KPI 카드 (HTML) + 버튼 클릭
    kc1,kc2,kc3,kc4,kc5,kc6=st.columns([1,1,1,1,1,0.35])
    kpi_list=[
        (kc1,"⏱️","총 손실",f"{total_min:,.1f}분 ({total_hr}h)","loss"),
        (kc2,"📍","분석 라인",f"{n_lines}개","line"),
        (kc3,"📅","분석 일수",f"{n_days}일","date"),
        (kc4,"📛","스크랩",f"{scrap_total:,}ea","scrap"),
        (kc5,"📋","스크랩 이력",f"{len(scrap_df):,}건","scrap_hist"),
    ]
    for col,icon,lbl,val,key in kpi_list:
        with col:
            st.markdown(f"""
            <div class="kpi-card">
        ...
        if total_df.empty:
            ...
            with col_l:
                ...
                if ct and ct.get("selection",{}).get("points"):
                    ...
                    if sn2:
                        ...
                        <b>📋 {sn2} 상세</b></div>""",unsafe_allow_html=True)
            ...
            with col_r:
                ...
                fig2=px.bar(ls,x="line",y="loss_min",
                            ...
                            height=500,labels={"loss_min":"손실(분)","line":"라인"})
                ...
                if cl and cl.get("selection",{}).get("points"):
                    ...
                    if sl:
                        st.markdown(f"""<div class="detail-box">
                        <b>📋 {sl} 라인 상세</b></div>""",unsafe_allow_html=True)
                        ld=fdf[fdf["line"]==sl][
            ...
            with col_pie:
                ...
                for bcol,proc in zip([b1,b2,b3],["AI","SMT","MI"]):
                    with bcol:
                        ...
                        if st.button(f"{'✅ ' if is_sel else ''}{proc}",
    ...
    with tab2:
        if total_df.empty:
            ...
            fig4=px.line(dt,x="date",y="loss_min",color="process",
                         color_discrete_map=PROC_COLOR,markers=True,height=360,
    ...
    with tab3:
        ...
        if slot_df.empty:
            ...
            fs.update_layout(margin=dict(l=0,r=0,t=10,b=0),
                             yaxis=dict(rangemode="tozero"))
            ...
            if sdf2.empty:
                ...
                with ca:
                    ...
                    fp.update_layout(
                        ...
                        yaxis=dict(rangemode="tozero"),
                        height=360,margin=dict(l=0,r=0,t=10,b=0),xaxis_tickangle=-30)
    ...
    with tab7:
        ...
        with cd1:
            dba=load_db()
            if not dba.empty:
                st.info(f"LOSSTIME: {len(dba):,}건 | {dba['date'].nunique()}일 | {dba['line'].nunique()}개 라인")
        ...
        with cd2:
            sdba=load_scrap_db()
            if not sdba.empty:
                st.info(f"SCRAP: {len(sdba):,}건 | {int(sdba['qty'].sum())}ea | {sdba['line'].nunique()}개 라인")
